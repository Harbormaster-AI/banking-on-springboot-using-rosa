...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "7642e805-e50a-472a-b771-7d08a6986148"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.35548590304413574"
...
resource "aws_db_subnet_group" "default" {
  name       = "bankingonspringboot-db-subnet"
...
#  name        = "bankingonspringboot-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application bankingonspringboot created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for bankingonspringboot and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "bankingonspringboot-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "bankingonspringboot"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_bankingonspringboot"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "bankingonspringboot instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/banking-on-springboot:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/bankingonspringboot theharbormaster/banking-on-springboot:latest"
    ]
  }
}
